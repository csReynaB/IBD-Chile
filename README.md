...
- `Data/exist.csv`
- `Data/fold.csv`
- `Metadata/IBD-Chile_metadata.csv`

Main output:

- `Data/IBD-Chile.parquet`
...
Rscript src/R/02-run_phiper_analysis.R \
  PROJECT_DIR=IBD-Chile \
  PARQUET_NAME=IBD-Chile.parquet \
...
Rscript src/R/03-render_phiper_reports.R \
  BASE_DIR=/absolute/path/to/IBD-Chile/results \
...
Rscript src/R/03-render_phiper_reports.R \
  BASE_DIR=/absolute/path/to/IBD-Chile/results \
  GROUP_COLS=group_test,group_cmb,Etiology
```
...
```bash
sbatch scripts/get_reports.sh /absolute/path/to/IBD-Chile/results group_test
```

Or for several grouping variables:

```bash
sbatch scripts/get_reports.sh /absolute/path/to/IBD-Chile/results group_test,group_cmb,Etiology
```
